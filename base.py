...
Quest_Action = Base.classes.Quest_Action
RoundSummaries = Base.classes.RoundSummaries
Zones = Base.classes.zones
Equipment = Base.classes.equipments
Treasures = Base.classes.treasures
Board = Base.classes.board
Rounds = Base.classes.rounds
...
session.commit()
team_ids = session.query(Teams.IdTeam).all()
...
races_ids = session.query(Races.IdRace).all()
...
actions_ids = session.query(Actions.IdAction).all()
...
buffs_ids = session.query(Buffs.IdBuff).all()
...
mod_ids = session.query(Modifiers.IdMod).all()
...
for x in range(15000):
    fields.append(Fields(XCoordinate=randint(1,10000),YCoordinate=randint(1,10000)))

session.add_all(fields)
session.commit()

fields_ids = session.query(Fields.IdField).all()

# FILL THE BUFFS TABLE
neibors = []
for x in range(25000):
    neibors.append(Board(Field_1 = random.choice(fields_ids),Field_2 = random.choice(fields_ids)))
...
for x in range(5000):
    characters.append(Characters(Name = random.choice(NAMES_CHAR), # +  str(randint(1,2939945)),
                                 Level = randint(1,99),
...
for x in range(25000):
    action_char.append(Character_Action(IdCharacter = random.choice(characters_ids), IdAction = random.choice(actions_ids)))
...
buff_char = []
for x in range(25000):
    buff_char.append(Character_Buff(IdCharacter = random.choice(characters_ids),IdBuff = random.choice(buffs_ids)))
session.add_all(buff_char)
session.commit()

quests_ids = session.query(Quests.IdQuest).all()
quest_act = []
for x in range(50000):
    quest_act.append(Quest_Action(IdAction = random.choice(actions_ids),IdQuest = random.choice(quests_ids)))
session.add_all(quest_act)
session.commit()


classes_ids = session.query(Classes.IdClass).all()
race_cla = []
for x in range(300):
    race_cla.append(Race_Class(IdRace = random.choice(races_ids),IdClass = randint(1,6)))
session.add_all(race_cla)
session.commit()


skills_ids = session.query(Skills.IdSkill).all()
act_skill = []
for x in range(3000):
    act_skill.append(Action_Skill(IdAction = random.choice(actions_ids),IdSkill = random.choice(skills_ids)))
...
char_quest = []
for x in range(30000):
    char_quest.append(Character_Quest(IdCharacter = random.choice(characters_ids),IdQuest = random.choice(quests_ids)))
session.add_all(char_quest)
session.commit()



specs_ids = session.query(Specializations.IdSpec).all()
char_spec = []
for x in range(30000):
    char_spec.append(Character_Specialization(IdCharacter = random.choice(characters_ids),IdSpec = random.choice(specs_ids)))
...
treasueres_ids = session.query(Treasures.IdTreasure).all()

treasure_ques = []
for x in range(10000):
    treasure_ques.append(Treasure_Quest(IdQuest = random.choice(quests_ids),IdTreasure = random.choice(treasueres_ids)))
...
treasure_fiel = []
for x in range(10000):
    treasure_fiel.append(Treasure_Field(IdField = random.choice(fields_ids),IdTreasure = random.choice(treasueres_ids)))
session.add_all(treasure_fiel)
session.commit()

rolls_ids = session.query(Rolls.IdRoll).all()

roll_skill = []
for x in range(10000):
    roll_skill.append(Roll_Skill(IdRoll = random.choice(rolls_ids),IdSkill = random.choice(skills_ids)))
session.add_all(roll_skill)
session.commit()



equips_ids = session.query(Equipments.IdEquipm).all()
zones_ids = session.query(Zones.IdZone).all()

eq_mod = []
for x in range(10000):
    eq_mod.append(Equipment_Modifer(IdMod = random.choice(mod_ids),IdEquipm=random.choice(equips_ids)))
...
eq_char = []
for x in range(10000):
    eq_char.append(Character_Equipment(IdCharacter = random.choice(characters_ids),IdEquipm= random.choice(equips_ids), Amount= randint(0,3)))
...
eq_tres = []
for x in range(10000):
    eq_tres.append(Treasure_Equipment(IdTreasure = random.choice(treasueres_ids),IdEquipm= random.choice(equips_ids)))
...
spec_skill = []
for x in range(30000):
    spec_skill.append(Specialization_Skill(IdSpec = random.choice(specs_ids),IdSkill= random.choice(skills_ids)))
...
char_zone = []
for x in range(1000):
    char_zone.append(Character_Zone_Equipment(IdCharacter = random.choice(characters_ids),IdZone = random.choice(zones_ids), IdEquipm =  random.choice(equips_ids)))
...
equ_zone = []
for x in range(1000):
    equ_zone.append(Equipment_Zone(IdEquipm=   random.choice(equips_ids), IdZone = random.choice(zones_ids)))
    print(x)
...
for x in range(20000):
    mod_buff.append(Buff_Modifier(ModifierValue = randint(-8,9),IdMod = random.choice(mod_ids), IdBuff = random.choice(buffs_ids)))
...
rounds_ids = session.query(Rounds.IdRound).all()

rounds_summaries = []
for x in range(60000):
    rounds_summaries.append(RoundSummaries(IdRound = random.choice(rounds_ids), IdRoll = random.choice(rolls_ids), IdCharacter=random.choice(IdCharacter), IdAction = random.choice(actions_ids), IdTarget=random.choice(fields_ids)))
